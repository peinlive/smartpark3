<?php
// /home/myzonaco/smartpark.myzona360.com/modules/parqueadero/asignar_uso.php
// v1.0 (3BI): Asignar apto USUARIO a una celda de parqueadero.
//
//   Modos:
...
//
//   Schema (verificado):
//     asignaciones_celdas: id, celda_id, apto_dueno_id, apto_usuario_id,
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['ver'])) {
    header('Content-Type: application/json; charset=utf-8');
    $celdaId = (int)($_GET['celda'] ?? 0);
    if ($celdaId < 1) { echo json_encode(['ok'=>false,'error'=>'Celda inválida']); exit; }
    try {
        $stC = $pdo->prepare("SELECT c.id, c.nombre_visible AS codigo, c.tipo,
                                     c.apto_dueno_id,
        ...
        if (!$cel) { echo json_encode(['ok'=>false,'error'=>'Celda no encontrada']); exit; }
        ...
        $stA = $pdo->prepare("SELECT ac.id, ac.tipo, ac.observacion,
                           ...
                           LEFT JOIN apartamentos au ON au.id = ac.apto_usuario_id
                               WHERE ac.celda_id = :cid AND ac.activa = 1 AND ac.archivado_en IS NULL
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        // Verificar que la celda pertenece al conjunto
        $stC = $pdo->prepare("SELECT c.id, c.nombre_visible, c.apto_dueno_id
                                FROM celdas c
                               WHERE c.id = :id AND c.conjunto_id = :c LIMIT 1");
        $stC->execute([':id' => $celdaId, ':c' => $conjuntoId]);
        $celda = $stC->fetch(PDO::FETCH_ASSOC);
        if (!$celda) { echo json_encode(['ok'=>false,'error'=>'Celda no encontrada']); exit; }

        $pdo->beginTransaction();

        // Archivar cualquier asignación activa previa (siempre, en ambos modos)
        $pdo->prepare("UPDATE asignaciones_celdas
                          SET activa = 0, archivado_en = NOW()
                        WHERE celda_id = :cid AND activa = 1 AND archivado_en IS NULL")
            ->execute([':cid' => $celdaId]);

        if ($accion === 'quitar') {
            // Solo archivar y listo
            if (function_exists('audit_log')) {
                audit_log('quitar_asignacion_uso', 'celdas', $celdaId,
                          "Se quitó la asignación de uso de la celda {$celda['nombre_visible']} (queda usada por el dueño)",
                          null, null);
            }
            $pdo->commit();
            echo json_encode(['ok'=>true, 'msg'=>'Asignación quitada. La celda queda usada por el dueño.']);
            exit;
        }
        ...
        // Si la celda no tiene apto_dueno_id definido, usar el mismo apto usuario como dueño
        ...
        $ins = $pdo->prepare("INSERT INTO asignaciones_celdas
                (celda_id, apto_dueno_id, apto_usuario_id, tipo,
        ...
        if (function_exists('audit_log')) {
            audit_log('asignar_uso_celda', 'celdas', $celdaId,
                      "Celda {$celda['nombre_visible']} asignada a apto {$aptoUsr['numero_visible']} ({$tipoAuto})",
                      null,
        ...
        echo json_encode([
            'ok'  => true,
            'msg' => "✅ Celda asignada a apto {$aptoUsr['numero_visible']} ({$etiqueta}).",
            'asignacion' => [
                'id'           => $nuevoId,
